<template>
  <view>
    <view
      v-show="ifshow"
      :style="{ top: navBar || !ifshow ? '0px' : navBarHeight + 'px' }"
      @tap="ableClose"
      @touchmove.stop.prevent
      class="popup-layer"
    >
    </view>
    <view
      ref="popRef"
      class="popup-content"
      @tap.stop="stopEvent"
      :style="_content_style"
    >
      <slot></slot>
    </view>
  </view>
</template>

<script>
// TODO: 将该组件用ts重写去掉用不到的代码，移到cpns中
import { AppModule } from "@/store/modules/app";

export default {
  name: "popup-layer",
  model: {
    prop: "showPop",
    event: "change",
  },
  props: {
    showPop: {
      type: Boolean,
      default: false,
    },
    direction: {
      type: String,
      default: "top", // 方向  top，bottom，left，right
    },
    autoClose: {
      type: Boolean,
      default: false,
    },
    fullScreen: {
      type: Boolean,
      default: false,
    },
    // 弹出层是否覆盖导航栏
    navBar: {
      type: Boolean,
      default: true,
    },
    backgroudColor: {
      type: String,
      default: "#FFFFFF",
    },
  },
  data() {
    return {
      ifshow: false, // 是否展示,
      translateValue: -100, // 位移距离
      timer: null,
      iftoggle: true,
    };
  },
  computed: {
    _translate() {
      const transformObj = {
        bottom: `transform:translateY(${-this.translateValue}%);`,
        top: `transform:translateY(${this.translateValue}%);`,
        left: `transform:translateX(${-this.translateValue}%);`,
        right: `transform:translateX(${this.translateValue}%);`,
      };
      return transformObj[this.direction];
    },
    _location() {
      const positionValue = {
        bottom: "bottom:0px;width:100%;",
        top: `top:${
          this.navBar || !this.ifshow ? 0 : this.navBarHeight
        }px;width:100%;`,
        left: "right:0px;height:100%;",
        right: "left:0px;height:100%;",
      };
      return positionValue[this.direction];
    },
    _content_style() {
      const height = this.fullScreen
        ? `height:calc(100% + ${this.navBar ? 0 : this.navBarHeight}px);`
        : "";
      return [
        `background-color:${this.backgroudColor};`,
        height,
        this._translate,
        this._location,
      ].join("");
    },
    navBarHeight() {
      return AppModule.navBarHeight;
    },
  },
  mounted() {
    if (this.showPop) {
      this.show();
    }
  },
  watch: {
    showPop(value) {
      console.log(value);
      if (value) {
        this.show();
      } else {
        this.close();
      }
    },
  },
  methods: {
    show(events) {
      this.ifshow = true;
      let _open = setTimeout(() => {
        this.translateValue = 0;
        _open = null;
      }, 100);
      let _toggle = setTimeout(() => {
        this.iftoggle = true;
        _toggle = null;
      }, 300);
    },
    close() {
      if (this.timer !== null || !this.iftoggle) {
        return;
      }
      this.translateValue = -100;
      this.timer = setTimeout(() => {
        this.ifshow = false;
        this.timer = null;
        this.iftoggle = false;
        this.$emit("closeCallBack", null);
        this.$emit("change", false);
      }, 300);
    },
    ableClose() {
      if (this.autoClose) {
        this.close();
      }
    },
    stopEvent(event) {},
  },
};
</script>

<style lang="scss">
.popup-layer {
  position: fixed;
  z-index: 999999;
  background: rgba(0, 0, 0, 0.3);
  height: 100%;
  width: 100%;
  top: 0px;
  left: 0px;
  overflow: hidden;
}

.popup-content {
  position: fixed;
  z-index: 1000000;
  background: #ffffff;
  transition: all 0.3s ease;
  overflow-x: hidden;
  overflow-y: scroll;
  // border:1px solid red;
}
</style>
